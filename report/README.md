...
**我的做法：**  
本次实验中实际采用 TF-IDF（词频 - 逆文档频率）替代 Word2Vec 进行文本特征表示：
1.TF-IDF 特征构建：使用sklearn.feature_extraction.text.TfidfVectorizer构建特征，设置最大特征数为 20000，采用 1-gram 和 2-gram 的词汇组合，子线性 TF 缩放（sublinear_tf=True）以平衡高频词汇的权重；
2.若使用 Word2Vec 方案：
词向量训练：基于预处理后的评论文本语料，训练 Word2Vec 模型，设置词向量维度为 100/200，窗口大小为 5，最小词频为 5；
句子向量生成：对每条评论的所有词汇对应的词向量进行均值池化，得到整条评论的句子向量，作为分类模型的输入特征。
---
...
**我的项目结构：**
words/
├── __pycache__/           # Python自动生成的字节码缓存，用于加速代码执行
│   └── main.cpython-312.pyc
├── .venv/                 # 项目专属虚拟环境，隔离Python依赖
│   ├── Include/
│   ├── Lib/
│   ├── Scripts/
│   └── pyvenv.cfg
...
│   └── README.md          # 实验报告主文档
├── results/               # 实验结果目录
│   └── Submission.csv     # Kaggle提交文件（含预测结果）
├── .env                   # 本地环境变量文件（不上传Git）
